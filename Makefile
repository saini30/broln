...
ESCPKG := github.com\/brolightningnetwork\/broln
...
DOCKER_TOOLS = docker run -v $$(pwd):/build broln-tools
...
$(brond_BIN):
	...
	cd $(TOOLS_DIR); go install -trimpath $(brond_PKG)
...
build:
	...
	$(GOBUILD) -tags="$(DEV_TAGS)" -o broln-debug $(DEV_GCFLAGS) $(DEV_LDFLAGS) $(PKG)/cmd/broln
	$(GOBUILD) -tags="$(DEV_TAGS)" -o brolncli-debug $(DEV_GCFLAGS) $(DEV_LDFLAGS) $(PKG)/cmd/brolncli
...
docker-tools:
	...
	docker build -q -t broln-tools $(TOOLS_DIR)
