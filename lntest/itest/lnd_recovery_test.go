...
import (
	...
	"github.com/brsuite/brond/bronutil/hdkeychain"
	"github.com/brolightningnetwork/broln/aezeed"
...
func testOnchainFundRecovery(net *lntest.NetworkHarness, t *harnessTest) {
	...
	skipAndSend := func(nskip int) func(*lntest.HarnessNode) {
		return func(node *lntest.HarnessNode) {
			...
			net.SendCoins(t.t, bronutil.SatoshiPerBitcoin, node)

			// And another to the next NP2WKH address.
			net.SendCoinsNP2WKH(
				t.t, bronutil.SatoshiPerBitcoin, node,
			)
	...
	// derivation path.
	restoreCheckBalance(3*bronutil.SatoshiPerBitcoin, 3, 1, skipAndSend(9))

	// Check that using a recovery window of 9 does not find the two most
	// recent txns.
	restoreCheckBalance(3*bronutil.SatoshiPerBitcoin, 3, 9, nil)
	...
	restoreCheckBalance(6*bronutil.SatoshiPerBitcoin, 6, 10, skipAndSend(19))

	// Check that recovering with a recovery window of 19 fails to find the
	// most recent transactions.
	restoreCheckBalance(6*bronutil.SatoshiPerBitcoin, 6, 19, nil)
	...
	const minerAmt = 8 * bronutil.SatoshiPerBitcoin
	const finalBalance = 9 * bronutil.SatoshiPerBitcoin
