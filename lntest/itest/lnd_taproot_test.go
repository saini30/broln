...
import (
	...
	"github.com/brsuite/brond/btcec/v2/schnorr"
...
func testTaprootComputeInputScriptKeySpendBip86(ctxt context.Context,
	...
	net.SendCoins(t.t, bronutil.SatoshiPerBitcoin, alice)
	...
	_, err = alice.SendCoins(ctxt, &lnrpc.SendCoinsRequest{
		...
		Amount: 0.5 * bronutil.SatoshiPerBitcoin,
	})
	require.NoError(t.t, err)
...
func testScriptHashLock(t *testing.T, preimage []byte) txscript.TapLeaf {
	...
	builder.AddData(bronutil.Hash160(preimage))
...
func newAddrWithScript(ctx context.Context, t *testing.T,
	node *lntest.HarnessNode, addrType lnrpc.AddressType) (bronutil.Address,
	[]byte) {
	...
	p2wkhAddr, err := bronutil.DecodeAddress(
...
func sendToTaprootOutput(ctx context.Context, t *harnessTest,
	...
	tapScriptAddr, err := bronutil.NewAddressTaproot(
...
func publishTxAndConfirmSweep(ctx context.Context, t *harnessTest,
	...
	txWeight := blockchain.GetTransactionWeight(bronutil.NewTx(tx))
...
func confirmAddress(ctx context.Context, t *harnessTest,
	...
	parsedAddr, err := bronutil.DecodeAddress(addrString, harnessNetParams)
	require.NoError(t.t, err)
