...
import (
	...
	"github.com/brolightningnetwork/broln/labels"
...
func createTestWallet(tempTestDir string, miningNode *rpctest.Harness,
	...
	cfg := lnwallet.Config{
		...
		DefaultConstraints: channeldb.ChannelConstraints{
			DustLimit:        500,
			MaxPendingAmount: lnwire.NewMSatFromSatoshis(bronutil.SatoshiPerBitcoin) * 100,
			ChanReserve:      100,
			MinHTLC:          400,
...
func testReservationInitiatorBalanceBelowDustCancel(miner *rpctest.Harness,
	...
	feePerKw := chainfee.SatPerKWeight(
		numBTC * numBTC * bronutil.SatoshiPerBitcoin,
	)
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:        chainHash,
		NodeID:           bobPub,
...
func testSingleFunderReservationWorkflow(miner *rpctest.Harness,
	...
	pushAmt := lnwire.NewMSatFromSatoshis(bronutil.SatoshiPerBitcoin)
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	if err != nil {
		t.Fatalf("unable to query fee estimator: %v", err)
	}
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	const numTxns = 5
	const outputAmt = bronutil.SatoshiPerBitcoin
...
func newTx(t *testing.T, r *rpctest.Harness, pubKey *btcec.PublicKey,
	...
	newOutput := &wire.TxOut{
		Value:    bronutil.SatoshiPerBitcoin,
		PkScript: keyScript,
	}
	tx, err := alice.SendOutputs(
	...
	txFee := bronutil.Amount(0.001 * bronutil.SatoshiPerBitcoin)
...
func testPublishTransaction(r *rpctest.Harness,
	...
	// transactions.
	var (
		txFee         = bronutil.Amount(0.005 * bronutil.SatoshiPerBitcoin)
...
func testSignOutputUsingTweaks(r *rpctest.Harness,
	...
	for i := 0; i < 2; i++ {
		...
		vm, err := txscript.NewEngine(
			...
			nil, nil, int64(bronutil.SatoshiPerBitcoin),
			txscript.NewCannedPrevOutputFetcher(
				keyScript, int64(bronutil.SatoshiPerBitcoin),
			),
		)
		if err != nil {
...
func testSpendUnconfirmed(miner *rpctest.Harness,
	...
	output = &wire.TxOut{
		Value:    bronutil.SatoshiPerBitcoin * 0.5,
		PkScript: alicePkScript,
	}
	...
	output = &wire.TxOut{
		Value:    int64(bobBalance) - (bronutil.SatoshiPerBitcoin * 0.4),
		PkScript: bobPkScript,
	}
...
func testSingleFunderExternalFundingTx(miner *rpctest.Harness,
	...
	// to Bob's side.
	chanAmt := 4 * bronutil.SatoshiPerBitcoin
