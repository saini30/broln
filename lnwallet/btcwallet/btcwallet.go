...
func (b *BtcWallet) AddressInfo(a bronutil.Address) (waddrmgr.ManagedAddress,
	error) {
...
func (b *BtcWallet) LeaseOutput(id wtxmgr.LockID, op wire.OutPoint,
	duration time.Duration) (time.Time, []byte, bronutil.Amount, error) {
	...
	for _, lease := range allLeases {
		if lease.Outpoint == op {
			return lockedUntil, lease.PkScript,
				bronutil.Amount(lease.Value), nil
		}
	}
...
func minedTransactionsToDetails(
	...
	for _, tx := range block.Transactions {
		...
		for i, txOut := range wireTx.TxOut {
			var addresses []bronutil.Address
