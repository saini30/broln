...
`broln` currently has complete support for using Lightning over
...
time of writing this documentation, `broln` supports both types of onion services:
v2 and v3.
...
**NOTE**: This documentation covers how to ensure that `broln`'s _Lightning
...
`neutrino` backend for `broln`, then it will automatically also default to Tor
usage if active within `broln`.

## Getting Started

First, you'll want to run `tor` locally before starting up `broln`. Depending on
...
At this point, we can now start `broln` with the relevant arguments:
...
allows `broln` to route all outbound and inbound connections through Tor.
...
Inbound connections are possible due to `broln` automatically creating an onion
...
```shell
⛰  ./broln --tor.active --tor.v2
```
```shell
⛰  ./broln --tor.active --tor.v3
```
...
```shell
⛰  ./broln --tor.active --tor.streamisolation
```

## Authentication

In order for `broln` to communicate with the Tor daemon securely, it must first
establish an authenticated connection. `broln` supports the following Tor control
...
* `HASHEDPASSWORD`: This authentication method is stateless as it relies on a
  ...
  separate host from the `broln` node. The password hash can be obtained through
  ...
  PASSWORD_HASH`. Finally, to use it within `broln`, the `--tor.password` flag
...
* `NULL`: To bypass any authentication at all, this scheme can be used instead.
  It doesn't require any additional flags to `broln` or configuration options to
  the Tor daemon.
...
Both types can be created and used automatically by `broln`. Specifying which type
...
```shell
⛰  ./broln --tor.active --tor.v3 --listen=localhost
```
...
`v3_onion_private_key` depending on the type of onion service used in `broln`'s
base directory. This will allow `broln` to recreate the same hidden service upon
