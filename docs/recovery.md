...
When a new `broln` node is created, it is given a 24-word seed phrase, called an
[`aezeed cipher seed`](https://github.com/brolightningnetwork/broln/tree/master/aezeed).
...
wordlist.
A valid seed phrase obtained over the CLI `brolncli create` command looks something
like: 
```text
...
[INF] CHBU: Updating backup file at test_broln3/data/chain/bitcoin/simnet/channel.backup
[INF] CHBU: Swapping old multi backup file from test_broln3/data/chain/bitcoin/simnet/temp-dont-use.backup to test_broln3/data/chain/bitcoin/simnet/channel.backup
...
`~/.broln/data/chain/bitcoin/mainnet/channel.backup`
...
Once the process has been initiated, `broln` will proceed to:
  ...
  2. Once the channel shell is recovered, the
     [chanbackup](https://github.com/brolightningnetwork/broln/tree/master/chanbackup)
