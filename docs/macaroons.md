...
`brolndir/data/chain/bitcoin/simnet/admin.macaroon`) by default and will error if
...
```shell
⛰  curl --insecure --header "Grpc-Metadata-macaroon: $(xxd -ps -u -c 1000  $HOME/.broln/data/chain/bitcoin/simnet/admin.macaroon)" https://localhost:8080/v1/getinfo
```
