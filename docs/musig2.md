...
With `broln v0.15.0-beta` a new, experimental MuSig2 API was added to the
...
**NOTE**: At the time the MuSig2 code in `brond`/`broln` was written, there was no
...
we take a look at the MuSig2 integration tests in `broln`, since they both serve
...
[`lntest/itest/broln_taproot_test.go`](../lntest/itest/broln_taproot_test.go) to see
the full code.
