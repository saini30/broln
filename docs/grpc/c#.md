...
Create a new `.net core` console application called `brobrolnclient` at your root directory (On Windows : `C:/`).

Create a folder `Grpc` in the root of your project and fetch the broln proto files

```shell
⛰  mkdir Grpc
⛰  curl -o Grpc/lightning.proto -s https://raw.githubusercontent.com/brolightningnetwork/broln/master/lnrpc/lightning.proto
```
...
Let's walk through some examples of C# `gRPC` clients. These examples assume that you have at least two `broln` nodes running, the RPC location of one of which is at the default `localhost:8998`, with an open channel between the two nodes.
...
Now, create an invoice for your node at `localhost:8998` and send a payment to it from another node.
```shell
...
```cs
// broln admin macaroon is at <broln_DIR>/data/chain/bitcoin/simnet/admin.macaroon on Windows
// ~/.broln/data/chain/bitcoin/simnet/admin.macaroon on Linux and ~/Library/Application Support/broln/data/chain/bitcoin/simnet/admin.macaroon on Mac
byte[] macaroonBytes = File.ReadAllBytes("<broln_DIR>/data/chain/bitcoin/simnet/admin.macaroon");
