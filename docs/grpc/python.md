...
5. Copy the broln lightning.proto file (you'll find this at
  ...
  or just download it
    ```shell
    broln ⛰  curl -o lightning.proto -s https://raw.githubusercontent.com/brolightningnetwork/broln/master/lnrpc/lightning.proto
    ```
...
```shell
broln ⛰  curl -o router.proto -s https://raw.githubusercontent.com/brolightningnetwork/broln/master/lnrpc/routerrpc/router.proto
...
channel = grpc.secure_channel('localhost:10008', creds)
...
is at the default `localhost:10008`, with an open channel between the two nodes.
...
Now, create an invoice for your node at `localhost:10008`and send a payment to
...
with open(os.path.expanduser('~/.broln/data/chain/bitcoin/simnet/admin.macaroon'), 'rb') as f:
...
channel = grpc.secure_channel('localhost:10008', combined_creds)
stub = lnrpc.LightningStub(channel)
