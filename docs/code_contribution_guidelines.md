...
- Changes to behavior within the daemon's interaction with the P2P protocol,
  ...
  [`broln_test.go`](https://github.com/brolightningnetwork/broln/blob/master/lntest/itest/broln_test.go).
...
When contributing to `broln` it is recommended to sign your git commits. This is
...
in `broln` to help improve the overall readability.
...
See [template](https://github.com/brolightningnetwork/broln/blob/master/.github/pull_request_template.md).
