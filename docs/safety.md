...
By default the two API connections `broln` offers (gRPC on port 10008 and REST on
...
Important to know:
* Deleting the `*.macaroon` files in the `<broln-dir>/data/chain/bitcoin/mainnet/`
...
There is
[a tool being worked on](https://github.com/brolightningnetwork/broln/pull/2373)
...
is the `<brolndir>/data/chain/bitcoin/mainnet/channel.backup` file which holds
...
`<brolndir>/data/chain/bitcoin/mainnet/channel.backup`
...
* If an external IP is set (either with `--externalip` or `--tlsextraip`) these
  ...
  point 1.
* If port `9736` (or `10008` for gRPC) was forwarded on the router, these
...
Multiple `broln` nodes can run off of a single `bitcoind` instance. There will be
connection/thread/performance limits at some number of `broln` nodes but in
practice running 2 or 3 `broln` instances per `bitcoind` node didn't show any
problems.
