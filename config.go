...
import (
	...
	"github.com/brolightningnetwork/broln/lnrpc/peersrpc"
...
var (
	...
	defaultBitcoindDir  = bronutil.AppDataDir("bitcoin", false)
...
type Config struct {
	...
	ExternalHosts     []string `long:"externalhosts" description:"Add a hostname:port that should be periodically resolved to announce IPs for. If a port is not specified, the default (9736) will be used."`
...
func LoadConfig(interceptor signal.Interceptor) (*Config, error) {
	...
	switch {
	// User specified --brolndir but no --configfile. Update the config file
	// path to the broln config directory, but don't require it to exist.
	case configFileDir != DefaultbrolnDir &&
...
func ValidateConfig(cfg Config, interceptor signal.Interceptor, fileParser,
	...
	case cfg.Bitcoin.Active:
		...
		default:
			str := "only brond, bitcoind, and neutrino mode " +
