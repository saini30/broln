...
func setup(t *testing.T, initialChans []LocalChannel) (*testContext, func()) {
	...
	var availableFunds bronutil.Amount = 10 * bronutil.SatoshiPerBitcoin
...
func TestAgentChannelOpenSignal(t *testing.T) {
	...
	newChan := LocalChannel{
		ChanID:  randChanID(),
		Balance: bronutil.SatoshiPerBitcoin,
	}
...
func TestAgentHeuristicUpdateSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     1 * bronutil.SatoshiPerBitcoin,
		},
	)
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * bronutil.SatoshiPerBitcoin})
...
func TestAgentChannelCloseSignal(t *testing.T) {
	...
	initialChans := []LocalChannel{
		...
		{
			ChanID:  randChanID(),
			Balance: bronutil.SatoshiPerBitcoin * 2,
		},
	}
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	testCtx.Lock()
	testCtx.walletBalance += bronutil.SatoshiPerBitcoin * 5
	testCtx.Unlock()
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: numChans,
			amt:     5 * bronutil.SatoshiPerBitcoin,
		},
	)
	...
	for i := 0; i < numChans; i++ {
		...
		case openChan := <-chanController.openChanSignals:
			if openChan.amt != bronutil.SatoshiPerBitcoin {
				t.Fatalf("invalid chan amt: expected %v, got %v",
					bronutil.SatoshiPerBitcoin, openChan.amt)
			}
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     bronutil.SatoshiPerBitcoin,
		},
	)
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * bronutil.SatoshiPerBitcoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, bronutil.SatoshiPerBitcoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	var channelBudget bronutil.Amount = 5 * bronutil.SatoshiPerBitcoin
	...
	channelBudget = bronutil.SatoshiPerBitcoin * 3 / 2
	...
	// our budget.
	channelBudget = bronutil.SatoshiPerBitcoin * 5
	numNewChannels = 2
	respondWithScores(
		t, testCtx, channelBudget, numExistingChannels,
		numNewChannels, nodeScores,
