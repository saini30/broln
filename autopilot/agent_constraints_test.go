...
func TestConstraintsChannelBudget(t *testing.T) {
	...
	const (
		minChanSize = 0
		maxChanSize = bronutil.Amount(bronutil.SatoshiPerBitcoin)
	...
	testCases := []struct {
		...
		// channels.
		{
			...
			bronutil.Amount(bronutil.SatoshiPerBitcoin * 10),
			false,
			0,
			0,
		},
		...
		// threshold.
		{
			[]LocalChannel{
				{
					ChanID:  randChanID(),
					Balance: bronutil.Amount(bronutil.SatoshiPerBitcoin),
				},
				{
					ChanID:  randChanID(),
					Balance: bronutil.Amount(bronutil.SatoshiPerBitcoin),
			...
			bronutil.Amount(bronutil.SatoshiPerBitcoin * 2),
			false,
			0,
			0,
		},
		...
		// limit is 3.
		{
			...
			bronutil.Amount(bronutil.SatoshiPerBitcoin * 9),
			true,
			bronutil.Amount(bronutil.SatoshiPerBitcoin * 4),
			2,
		},
		...
		// channel as the limit is 3.
		{
			[]LocalChannel{
				...
				{
					ChanID:  randChanID(),
					Balance: bronutil.Amount(bronutil.SatoshiPerBitcoin * 3),
				},
			},
			bronutil.Amount(bronutil.SatoshiPerBitcoin * 10),
			true,
			bronutil.Amount(bronutil.SatoshiPerBitcoin * 3),
			1,
		},
		...
		// threshold.
		{
			...
			bronutil.Amount(bronutil.SatoshiPerBitcoin),
			false,
			0,
			0,
		},
