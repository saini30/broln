...
next versions will use the started `brond` bitcoin node in `testnet` and
...
+ ----- +                   + --- +         create channels and interact with each other using the   
    ...
    + - - - -  - + - - - - - - +            
        ...
        + --------------- +        "brond" node which represents the Bitcoin network, in a 
...
⛰  docker exec -it brond /start-btcctl.sh generate 400

# Check that segwit is active:
⛰  docker exec -it brond /start-btcctl.sh getblockchaininfo | grep -A 1 segwit
```
...
⛰  docker exec -it brond /start-btcctl.sh generate 3
