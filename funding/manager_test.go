...
import (
	...
	"github.com/brolightningnetwork/broln/lntest/mock"
...
func TestFundingManagerFundAll(t *testing.T) {
	...
	allCoins := []*lnwallet.Utxo{
		{
			...
			Value: bronutil.Amount(
				0.05 * bronutil.SatoshiPerBitcoin,
			),
		...
		},
		{
			AddressType: lnwallet.WitnessPubKey,
			Value: bronutil.Amount(
				0.06 * bronutil.SatoshiPerBitcoin,
			),
			PkScript: mock.CoinPkScript,
			OutPoint: wire.OutPoint{
				Hash:  chainhash.Hash{},
	...
	tests := []struct {
	...
	}{
		{
			...
			spendAmt: bronutil.Amount(
				0.11 * bronutil.SatoshiPerBitcoin,
			),
			change: false,
		},
		{
			...
			spendAmt: bronutil.Amount(
				0.10 * bronutil.SatoshiPerBitcoin,
			),
			change: true,
		},
	}
...
func TestMaxChannelSizeConfig(t *testing.T) {
	...
	// Attempt to create a channel above the limit
	// imposed by --maxchansize, which should be rejected.
	initReq.LocalFundingAmt = bronutil.SatoshiPerBitcoin + 1
...
func TestWumboChannelConfig(t *testing.T) {
	...
	// should be rejected.
	initReq.LocalFundingAmt = bronutil.SatoshiPerBitcoin
