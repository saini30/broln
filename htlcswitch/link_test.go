...
import (
	...
	"github.com/brolightningnetwork/broln/htlcswitch/hodl"
	...
	"github.com/brolightningnetwork/broln/lnpeer"
	...
	"github.com/brolightningnetwork/broln/ticker"
...
func TestChannelLinkRevThenSig(t *testing.T) {
	...
	const chanAmt = bronutil.SatoshiPerBitcoin * 5
	const chanReserve = bronutil.SatoshiPerBitcoin * 1
...
func TestChannelLinkSingleHopPayment(t *testing.T) {
	...
	alice, bob, cleanUp, err := createTwoClusterChannels(
		bronutil.SatoshiPerBitcoin*3,
		bronutil.SatoshiPerBitcoin*5)
	if err != nil {
	...
	amount := lnwire.NewMSatFromSatoshis(bronutil.SatoshiPerBitcoin)
...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	const amount = bronutil.SatoshiPerBitcoin
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	amountNoFee := lnwire.NewMSatFromSatoshis(bronutil.SatoshiPerBitcoin)
...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		bronutil.SatoshiPerBitcoin*5, bronutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// rejected.
	amount := lnwire.NewMSatFromSatoshis(4 * bronutil.SatoshiPerBitcoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func TestChannelLinkTrimCircuitsPending(t *testing.T) {
	...
	const (
		chanAmt   = bronutil.SatoshiPerBitcoin * 5
		numHtlcs  = 4
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	// a switch initiated payment.  The resulting bandwidth should
	// now be decremented to reflect the new HTLC.
	htlcAmt := lnwire.NewMSatFromSatoshis(3 * bronutil.SatoshiPerBitcoin)
	...
	const bobChanAmt = bronutil.SatoshiPerBitcoin * 1
	const bobChanReserve = bronutil.SatoshiPerBitcoin * 1.5
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	// for this test.
	const aliceInitialBalance = bronutil.SatoshiPerBitcoin * 3
	channels, cleanUp, _, err := createClusterChannels(
		aliceInitialBalance, bronutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
